  ...
  return <div className="min-h-screen pb-24 md:pb-0"><Header bagCount={cart.count} onBagOpen={() => navigate({ to: "/checkout" })} />
    <main>
      <section className="grid border-b border-foreground/15 lg:grid-cols-[1.08fr_.92fr]">
        ...
        <div className="flex px-6 py-12 md:px-12 lg:sticky lg:top-16 lg:min-h-[calc(100vh-4rem)] lg:items-center lg:px-16"><div className="w-full"><p className="header-label text-muted-foreground">COCO HONEY BRONZE · 62 ML</p><h1 className="mt-5 text-5xl font-semibold uppercase leading-none md:text-7xl">{active.name}</h1><img src={script} alt="The secret ingredient is always love" className="mt-6 h-auto w-full max-w-md" /><a href="#avaliacoes" className="mt-6 flex items-center gap-2 text-xs text-muted-foreground"><Stars rating={reviewStats.average} /><span className="underline underline-offset-4">{reviewStats.average.toFixed(1)} · {reviewStats.total} avaliações</span></a><p className="mt-5 max-w-xl leading-relaxed text-muted-foreground">Óleo acelerador de bronzeado natural e vegano. Textura leve, aplicação uniforme, resistente à água e ao suor.</p>
          <div className="mt-8 grid grid-cols-3 gap-2">{packs.map((pack) => <Button key={pack.slug} variant={pack.slug === selected ? "brand" : "outline"} className="h-auto rounded-lg px-2 py-3" onClick={() => setSelected(pack.slug)}><span><span className="block text-xs font-semibold">PACK {pack.shortName}</span><span className="mt-1 block text-[10px] font-normal">{pack.price}</span><span className="mt-0.5 block text-[9px] font-normal opacity-70">{perBottle(pack) ? `${perBottle(pack)?.replace(",00", "")}/un` : "1 frasco"}</span></span></Button>)}</div>
          <div className="mt-8 flex items-end justify-between"><div>{active.comparePrice ? <p className="text-sm text-muted-foreground line-through">{active.comparePrice}</p> : null}<p className="text-3xl font-semibold">{active.price}</p>{perBottle(active) ? <p className="mt-1 text-sm font-medium">{perBottle(active)} por frasco</p> : null}<p className="mt-1 text-xs text-muted-foreground">{installmentText(active.price)}</p></div><div className="text-right">{active.badge ? <span className="header-label block">{active.badge}</span> : null}{hasFreeShipping(active) ? <span className="header-label mt-2 flex items-center justify-end gap-1.5"><Truck className="h-3 w-3" />FRETE GRÁTIS</span> : null}</div></div>
          <Button variant="brand" className="mt-6 h-14 w-full rounded-lg text-xs font-semibold uppercase" onClick={add} disabled={active.soldOut}><ShoppingBag />{active.soldOut ? "Esgotado" : "Adicionar à sacola"}</Button>
          <ShippingQuote slug={selected} />
          <div className="mt-8 grid grid-cols-2 gap-3 text-xs">{["Natural", "Vegano", "Não testado em animais", "Sem conservantes"].map((item) => <span key={item} className="flex items-center gap-2 border-t border-foreground/15 pt-3"><Check className="h-3 w-3" />{item}</span>)}</div>
        </div></div>
      </section>
