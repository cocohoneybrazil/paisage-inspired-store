    ...
    <main>
      ...
      <section id="packs" className="border-b border-foreground/15 bg-background">
        <div className="mx-auto grid max-w-[1600px] lg:grid-cols-2">
          <div className="flex items-center px-6 py-16 md:px-12 lg:px-16"><div className="max-w-xl"><p className="header-label text-muted-foreground">MAIS VENDIDO · MELHOR CUSTO-BENEFÍCIO</p><h2 className="mt-6 text-5xl font-semibold uppercase leading-none md:text-7xl">Bronze pro verão inteiro</h2><p className="mt-8 text-lg leading-relaxed text-muted-foreground">Coco Honey Bronze · óleo acelerador · 3 un · 186 ml · 10% de desconto + Sun Bag. Três vidros de 62 ml para segurar o bronze da primeira praia até o último feriado — e a Sun Bag CHB de brinde.</p><ul className="mt-8 divide-y divide-foreground/15 border-y border-foreground/15">{["10% de desconto no combo + Sun Bag CHB", "Frete grátis", "186 ml — cerca de 3 meses de uso", "Fórmula natural e vegana, sem oxibenzona"].map((benefit) => <li key={benefit} className="py-4 text-sm font-medium">{benefit}</li>)}</ul><div className="mt-8 flex flex-wrap items-baseline gap-x-4 gap-y-2"><span className="flex items-baseline gap-1"><span className="text-sm text-muted-foreground line-through">{featured.comparePrice}</span><span className="text-2xl font-semibold">{featured.price}</span></span><span className="text-sm text-muted-foreground">{installmentText(featured.price)}</span></div><div className="mt-5 flex flex-wrap items-center gap-5"><Button variant="brand" size="lg" className="h-12 rounded-lg px-7 text-xs font-semibold uppercase" onClick={() => add()}><ShoppingBag />Quero o Pack 003</Button><Link to="/produto" search={{ pack: featured.slug }} className="text-xs font-semibold uppercase underline underline-offset-4">Ver detalhes</Link></div></div></div>
